...
NSString* processPurpleImages(NSString* inString, AIAccount* adiumAccount)
{
    ...
    while (![scanner isAtEnd]) {
		...
		if ([scanner scanString:targetString intoString:&chunkString]) {
			...
			PurpleStoredImage		*purpleImage = purple_imgstore_get(imageID);
			if (purpleImage) {
...
- (NSString *)localizedPurpleString:(NSString *)inString
{
	...
	if (!configuredGettext) {
		bindtextdomain("libpurple", [[[[NSBundle bundleForClass:[self class]] resourcePath] stringByAppendingPathComponent:@"potfiles"] UTF8String]);
		bind_textdomain_codeset("libpurple", "UTF-8");
	...
	return [NSString stringWithUTF8String:dgettext("libpurple", [inString UTF8String])];
}
*/
