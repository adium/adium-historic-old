...
- (void)doStreamOpen
{
	...
	if ([self areStreamsConnected] && !(theFlags & kDidCallConnectDeleg)) {
		...
		if ([theDelegate respondsToSelector:@selector(onSocket:didConnectToHost:port:)])
			[theDelegate onSocket:self didConnectToHost:[self addressHost:peer] port:[self addressPort:peer]];
		CFRelease (peer);
...
- (void) doSendBytes
{
	if (theCurrentWrite != nil && theWriteStream != NULL)
	{
		...
		while (!done && !error && CFWriteStreamCanAcceptBytes (theWriteStream))
		{
			...
			[self didSendDataWithLength:bytesWritten];
...
{
	if ([theDelegate respondsToSelector:@selector(didSendDataWithLength:)])
	{
		[theDelegate didSendDataWithLength:length];
	}
}
