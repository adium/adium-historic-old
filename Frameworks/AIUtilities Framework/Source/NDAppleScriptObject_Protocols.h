 ...
 * @brief Header file for the protocols <tt>NDAppleScriptObjectSendEvent</tt> and <tt>NDAppleScriptObjectActive</tt>
 *
* The protocols <tt>NDAppleScriptObjectSendEvent</tt> and <tt>NDAppleScriptObjectActive</tt> are used by <tt>NDAppleScriptObject</tt>
 */
 ...
 * @brief Protocol used by <tt><a href="../../../NDAppleScriptObject/index.html" target="_top">NDAppleScriptObject</a></tt>
 *
* The protocol <tt>NDAppleScriptObjectSendEvent</tt> is implemented by any object that wishes to act as an AppleEvent send target as passed to the method <tt>setAppleEventSendTarget:</tt>.
 */
 ...
 * @brief Passes an event to be executed by a AppleScript to the receiver
 *
* Called every time the AppleScript is about to send an AppleEvent. The method <tt>sendAppleEvent:sendMode:sendPriority:timeOutInTicks:idleProc:filterProc:</tt> can be used to intercept every AppleEvent that the AppleScript would send so that it can be modified or redirected. For example AppleEvent for GUI interaction like &ldquo;display dialog&rdquo; need to be sent within the main run loop and thread. The class <tt>NDAppleScriptObject</tt> implements this protocol it&rsquo;s self and so you can pass this message onto your <tt>NDAppleScriptObject</tt> object.
 ...
 * @return A <tt>NSAppleEventDescriptor</tt> contain the result.
  */
 ...
 * @brief Method called periodical during script execution.
 *
* Allows you to do some processing durring execution of a AppleScript with the same thread.
 * @return Returns <tt>NO</tt> to specify some error occurred.
  */
