...
- (void)drawBackgroundGradientInRect:(NSRect)inRect
{
	...
	//Add a sealing line at the light side of the gradient to make it look more
	//	polished.  Apple does this with most gradients in OS X.
	float gradientL, backgroundL;
	[gradientColor   getHue:NULL luminance:&gradientL   saturation:NULL];
	[backgroundColor getHue:NULL luminance:&backgroundL saturation:NULL];

	NSColor *colors[2] = { backgroundColor, gradientColor  };
	unsigned which = (gradientL < backgroundL);
	[colors[which] set];
	[NSBezierPath fillRect:NSMakeRect(inRect.origin.x, inRect.origin.y + (inRect.size.height * which) - which, inRect.size.width, 1)];
}
