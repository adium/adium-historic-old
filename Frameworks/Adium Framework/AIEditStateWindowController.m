...
- (id)initWithWindowNibName:(NSString *)windowNibName customState:(NSDictionary *)inState notifyingTarget:(id)inTarget;
...
+ (void)editCustomState:(NSDictionary *)state onWindow:(id)parentWindow notifyingTarget:(id)inTarget
{
	...
	controller = [[self alloc] initWithWindowNibName:@"EditStateSheet" customState:state notifyingTarget:inTarget];
...
- (void)windowDidLoad
{
	...
	[self configureForState:originalState];
}
...
- (IBAction)okay:(id)sender
{
	if(target && [target respondsToSelector:@selector(customStatusState:changedTo:)]){
		[target customStatusState:originalState changedTo:[self currentConfiguration]];
	}
...
{
	//Availability
	[checkbox_away setEnabled:![checkbox_available state]];
	[checkbox_available setEnabled:![checkbox_away state]];
	[checkbox_customAutoReply setEnabled:[checkbox_away state]];
	
	//Visibility
	[scrollView_available setHidden:![checkbox_available state]];
	[scrollView_away setHidden:![checkbox_away state]];
	...
	id	current = checkbox_away;
	int	height = WINDOW_HEIGHT_PADDING;
	
	current = [self _positionControl:scrollView_away relativeTo:current height:&height];
	...
	current = [self _positionControl:checkbox_available relativeTo:current height:&height];
	current = [self _positionControl:scrollView_available relativeTo:current height:&height];
...
{
	//Toggles
	[checkbox_available setState:[[state objectForKey:STATE_AVAILABLE] boolValue]];
	[checkbox_invisible setState:[[state objectForKey:STATE_INVISIBLE] boolValue]];
	[checkbox_idle setState:[[state objectForKey:STATE_IDLE] boolValue]];
	[checkbox_away setState:[[state objectForKey:STATE_AWAY] boolValue]];
	[checkbox_autoReply setState:[[state objectForKey:STATE_AUTO_REPLY] boolValue]];
	[checkbox_customAutoReply setState:[[state objectForKey:STATE_AUTO_REPLY_IS_AWAY] boolValue]];
	
	//Strings
	[[textView_away textStorage] setAttributedString:[NSAttributedString stringWithData:[state objectForKey:STATE_AWAY_MESSAGE]]];
	[[textView_available textStorage] setAttributedString:[NSAttributedString stringWithData:[state objectForKey:STATE_AVAILABLE_MESSAGE]]];
	[[textView_autoReply textStorage] setAttributedString:[NSAttributedString stringWithData:[state objectForKey:STATE_AUTO_REPLY_MESSAGE]]];
	
	//Idle start
	int	idleStart = [[state objectForKey:STATE_IDLE_START] intValue];
...
{
	int	idleStart = [textField_idleHours intValue]*3600 + [textField_idleMinutes intValue]*60;
	
	return([NSDictionary dictionaryWithObjectsAndKeys:
		STATE_SAVED_STATE, STATE_TYPE,
		@"", STATE_TITLE,
		[NSNumber numberWithBool:[checkbox_away state]], STATE_AWAY,
		[NSNumber numberWithBool:[checkbox_autoReply state]], STATE_AUTO_REPLY,
		[NSNumber numberWithBool:[checkbox_customAutoReply state]], STATE_AUTO_REPLY_IS_AWAY,
		[NSNumber numberWithBool:[checkbox_invisible state]], STATE_INVISIBLE,
		[NSNumber numberWithBool:[checkbox_available state]], STATE_AVAILABLE,
		[NSNumber numberWithBool:[checkbox_idle state]], STATE_IDLE,
		[NSNumber numberWithInt:idleStart], STATE_IDLE_START,
		[[textView_away textStorage] dataRepresentation], STATE_AWAY_MESSAGE,
		[[textView_available textStorage] dataRepresentation], STATE_AVAILABLE_MESSAGE,
		[[textView_autoReply textStorage] dataRepresentation], STATE_AUTO_REPLY_MESSAGE,
		nil]);
}
