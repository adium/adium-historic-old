...
- (void)installPlugin{
    ...
    //Install Menu item
    versionCheckerMenuItem = [[[NSMenuItem alloc] initWithTitle:VERSION_CHECKER_TITLE target:self action:@selector(checkForNewVersion:) keyEquivalent:@""] autorelease];
...
{   
    //Grab the current buildDate from our buildnum script
	   ...
	   if(path = (char *)[[[NSBundle mainBundle] pathForResource:@"buildnum" ofType:nil] fileSystemRepresentation])
	   {
		   ...
		   [NSURL URLWithString:@"http://adium.sourceforge.net/version.plist"]];
