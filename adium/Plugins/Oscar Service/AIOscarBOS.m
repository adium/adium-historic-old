...
@interface AIOscarBOS (PRIVATE)
- (void)_handleBOSRights:(AIOscarPacket *)inPacket;
@end

@implementation AIOscarBOS
...
{
    [connection sendPacket:[connection snacPacketWithFamily:0x0009 type:0x0002 flags:0x000]]; 
}
...
{
    switch(type){
        case 0x0003: [self _handleBOSRights:inPacket]; break;
...
- (void)_handleBOSRights:(AIOscarPacket *)inPacket
{
    NSLog(@"_handleBOSRights (Not implemented)"); //Not implemented
    //AIOscarTLVBlock	*valueBlock = [inPacket getTLVBlock];

    //NSLog(@"Max Buddies:%i",[valueBlock integerForType:0x0001]);
    //NSLog(@"Max Denies:%i",[valueBlock integerForType:0x0002]);
}
