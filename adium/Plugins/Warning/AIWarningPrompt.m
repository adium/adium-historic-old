...
#import "AIWarningPrompt.h"
#import "AIAdium.h"
#import <AIUtilities/AIUtilities.h>
#import <Adium/Adium.h>

#define WARNING_PROMPT_NIB	@"WarningPrompt"

@interface AIWarningPrompt (PRIVATE)
...
@implementation AIWarningPrompt

static AIWarningPrompt *sharedInstance = nil;
+ (void)warningPromptWithOwner:(id)inOwner
{
    if(!sharedInstance){
        sharedInstance = [[self alloc] initWithWindowNibName:WARNING_PROMPT_NIB owner:inOwner];
    }
...
- (IBAction)warning:(id)sender
{
    ...
    if(contact){
        //Close the prompt
        [AIWarningPrompt closeSharedInstance];

        //Initiate the warning
        /*[[owner notificationCenter] postNotificationName:???????_InitiateWarning
                                                  object:nil
