	...
	on perform action for aPerson with screenName
		...
		using terms from application "Adium"
			...
			tell application "Adium"
				...
				if (((every «class acct» whose «class AsSC» is "Jabber" and «class AcOn» is yes) count) is equal to 0) then
					tell (the first «class acct» whose «class AsSC» is "Jabber") to «event AdIMcnct»
				end if
				
				(* Create the chat and find the contact it is with *)
				tell the first «class astC» to «event AdIMcCdM» given «class TO  »:screenName, «class cCsI»:"Jabber"
				set theContact to (the first «class ltct» whose «class AUID» is screenName and «class AsSC» is "Jabber")
